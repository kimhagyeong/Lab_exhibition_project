...
##style 3
style_model.load_params('/home/pyj/style/MXNet-Gluon-Style-Transfer-master/models_1024_10e/Epoch_6iters_120000.params', ctx=ctx)
...
def rvevaluate(input, id):
    ...
    # images
    content_image = tensor_load_rgbimage_img(input,ctx, size=1024, keep_asp=True)
    content_image1 = tensor_load_rgbimage_img(input,ctx1, size=1024, keep_asp=True)
    content_image2 = tensor_load_rgbimage_img(input,ctx2, size=1024, keep_asp=True)
    
    #미리 불러놓고 쓰기
    style_image = tensor_load_rgbimage(f"images/style1/{id}.jpg", ctx, size=512)
    style_image1 = tensor_load_rgbimage(f"images/style1/{id}.jpg", ctx1, size=512)
    style_image2 = tensor_load_rgbimage(f"images/style1/{id}.jpg", ctx2, size=512)
